...
Base_datos_grafico <- Base_datos %>%
  # Creamos la variable 'Tiempo_trabajado_Anual'
  mutate(Tiempo_trabajado_Anual = case_when(
    Tiempo_trabajado <= 12 ~ "0-1 Año",
    Tiempo_trabajado > 12 & Tiempo_trabajado <= 24 ~ "1-2 Años",
    Tiempo_trabajado > 24 & Tiempo_trabajado <= 36 ~ "2-3 Años",
    Tiempo_trabajado > 36 & Tiempo_trabajado <= 60 ~ "3-5 Años",
    Tiempo_trabajado > 60 ~ "Más de 5 Años",
    TRUE ~ "No aplica/Otro" # Para valores NA o fuera de rango
  )) %>%
  # Convertir la nueva variable a factor para ordenarla correctamente en el gráfico
  mutate(Tiempo_trabajado_Anual = factor(Tiempo_trabajado_Anual, 
                                         levels = c("0-1 Año", "1-2 Años", "2-3 Años", "3-5 Años", "Más de 5 Años", "No aplica/Otro")))

# 2. Generar el gráfico de cajas y bigotes con la variable agrupada
ggplot(Base_datos_grafico, aes(x = Tiempo_trabajado_Anual, y = `Ingreso del hogar`)) +
  geom_boxplot(fill = "#0072B2", color = "darkblue", outlier.alpha = 0.1) +
  labs(
    title = "Logaritmo del Ingreso del Hogar por Antigüedad Laboral",
    subtitle = "La línea horizontal es la mediana del ingreso en cada grupo.",
    x = "Antigüedad Laboral (Años)",
